...
namespace MsgDAL
{
    public class MongoClient : MongoDB.Driver.MongoClient
    {
        public MongoClient()
        {
            string connStr = ConfigurationManager.ConnectionStrings["ohyconn"].ConnectionString;
            var x = new MongoDB.Driver.MongoClient(connStr);
        ...
        public List<T> FindPage<T>(Expression<Func<T, bool>> filter, int pageIndex, out long total, int pageSize = 10) where T : ModelBase
        {
            var coll = _ohyDB.GetCollection<T>(typeof(T).Name);
